<template>
  <div class="order-confirm">
    ...
    <!-- svg矢量图，拉取的时候不是一个图片地址，而是一段代码，不占用带宽，用svg代替png，减少请求数量  ，优化性能 -->
    ...
    <!-- 删除确认弹框 -->
    <!-- @cancel自定义事件 等价于 v-on:cancel="showDelModal=false" : 点击cancel时modal子组件会根据emit触发下面父组件submit自定义的函数，参考modal.vue 21行 -->
    <modal
      title="删除确认"
      btnType="1" 
...
export default{
  ...
  data(){
    return {
      list:[],//收货地址列表
      cartList:[],//购物车中需要结算的商品列表，不是购物车所有商品
      cartTotalPrice:0,//商品总金额
      count:0,//商品结算数量，要购买的商品总件数，要遍历计算
      checkedItem:{},//选中的地址对象
  ...
  methods:{
    getAddressList(){//获取地址列表,查找收货地址接口文档
      this.axios.get('/shippings').then((res)=>{
        this.list = res.list;//这里的res在main.js里面返回的就是res.data，所以不能写成res.data.list
      })
    },
    // 新增地址
    openAddressModal(){
    ...
    delAddress(item){
      this.checkedItem = item;//选中的地址
      this.userAction = 2;//用户行为是删除地址
      this.showDelModal = true;//显示确认删除的弹框
    },
    // 地址删除、编辑、新增功能
    submitAddress(){
      let {checkedItem,userAction} = this;//解构出来
      let method,url,params={};
      if(userAction == 0){
        method = 'post',url = '/shippings';
      }else if(userAction == 1){
        method = 'put',url = `/shippings/${checkedItem.id}`;//字符串模板
      }else {
        method = 'delete',url = `/shippings/${checkedItem.id}`;
      }
      if(userAction == 0 || userAction ==1){//新增或者编辑
        let { receiverName, receiverMobile, receiverProvince, receiverCity, receiverDistrict, receiverAddress, receiverZip} = checkedItem;
        let errMsg='';
        //错误提示
        if(!receiverName){
          errMsg = '请输入收货人名称';
        }else if(!receiverMobile || !/\d{11}/.test(receiverMobile)){//正则判断，手机号必须是11位数字
          errMsg = '请输入正确格式的手机号';
          ...
          errMsg = '请输入收货地址';
        }else if(!/\d{6}/.test(receiverZip)){//正则判断，邮编必须是6位数字，vue中正则语法：https://blog.csdn.net/xuan0555/article/details/111317582
          errMsg = '请输入六位邮编';
        }
        if(errMsg){
      ...
      this.axios[method](url,params).then(()=>{//method是动态获取的
        this.closeModal();
        this.getAddressList();//重新拉取最新的地址列表
    ...
    getCartList(){
      ...
      this.axios.get('/carts').then((res)=>{
        ...
        this.cartList = list.filter(item=>item.productSelected);//过滤出item.productSelected=True的item,返回已经选中了的要购买的商品列表
        this.cartList.map((item)=>{//遍历所有要结算的商品
          this.count += item.quantity;//每个要购买商品的数量相加
        })
      })
    },
    // 订单提交
    orderSubmit(){
      let item = this.list[this.checkIndex];//选中的地址
      if(!item){//没有选中地址
      ...
      this.axios.post('/orders',{
        ...
        this.$router.push({
          path:'/order/pay',
          query:{//跳转后的页面会拼上路由参数：订单号orderNo
            orderNo:res.orderNo
          }
        })
      })
    ...
    .edit-wrap{
      ...
      .item{
        ...
        .input{
          ...
          padding-left:15px;
          border:1px solid #E5E5E5;
          // 相邻兄弟选择器：https://www.w3school.com.cn/css/css_selector_adjacent_sibling.asp
          &+.input{//第二个input框。显示手机号，他是第一个input（姓名）的下一个相邻兄弟元素
            margin-left:14px;
          }
        }
        select{
